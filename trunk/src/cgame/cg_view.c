...
static void CG_ZoomSway( void ) {
	...
	spreadfrac = (float)cg.predictedPlayerState.aimSpreadScale / 255.0;
...
static int CG_CalcFov( void ) {
	...
	// CHRUKER: b088 - Players field of view at intermission is different if you
	// had deployed a MG42 (fixation moved further down)
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
		...
		if(CG_BackupProfile()) {
			for(i = 0; i < cg.forceCvarCount; i++) {
				...
				trap_Cvar_Register(NULL,
					...
					cg.forceCvars[i][1],
					CVAR_ROM);
				trap_Cvar_Set(cg.forceCvars[i][0],
					cg.forceCvars[i][1]);
			}
			CG_RestoreProfile();
		}
	...
	/* // kw: this is already checked in CG_DrawCrosshair
	if (cg.weaponSelect == WP_FG42SCOPE) {
		float spd;
	...
	if(cg.mvCurrentMainview != NULL && cg.snap->ps.pm_type != PM_INTERMISSION) {
		...
		if(cg.predictedPlayerState.grenadeTimeLeft) {	// grenades and dynamite set this
			...
			if( cg.predictedPlayerState.weapon == WP_DYNAMITE ) {
