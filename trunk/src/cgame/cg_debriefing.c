...
#define DB_MASTER_FONT &cgs.media.font3
...
panel_button_t debriefTitleWindow = {
	...
	NULL,	/* keyDown	*/
	NULL,	/* keyUp	*/
	CG_Debriefing2_Title_Draw,	// kw: renamed
	NULL,
};
...
panel_button_t debriefMissionTitleWindow = {
	...
	NULL,	/* keyDown	*/
	NULL,	/* keyUp	*/
	CG_Debriefing2_MissionTitle_Draw,
	NULL,	
};
...
panel_button_t mapVoteNamesList = {
	NULL,
	NULL,
	{ DB_MAPNAME_X, DB_MAPVOTE_Y, 250, 240 },
	{ 0, 0, 0, 0, 0, 0, 0, 0 },
	&mapVoteFont,				/* font		*/
...
panel_button_t mapVoteNamesListScroll = {
	NULL,
	NULL,
	{ DB_MAPVOTE_X + 50, DB_MAPVOTE_Y, 16, 240 },
	{ 3, 0, 0, 0, 0, 0, 0, 0 },
	NULL,		/* font		*/
	CG_Debriefing_Scrollbar_KeyDown,	/* keyDown	*/
	CG_Debriefing_Scrollbar_KeyUp,		/* keyUp	*/
...
void CG_Debriefing_ChatBox_Draw( panel_button_t* button ) {
	...
	if( cgs.teamLastChatPos != cgs.teamChatPos ) {
		...
		for( i = cgs.teamLastChatPos; i < cgs.teamChatPos; i++ ) {
			len = CG_Text_Width_Ext( cgs.teamChatMsgs[i % chatHeight], 0.2f, 0, &cgs.media.font3 );
			if( len > w ) {
				w = len;
			}
		}
		...
		for( i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i-- ) {
			...
			CG_Text_Paint_Ext( button->rect.x + 12, button->rect.y - (cgs.teamChatPos - i - 1)*lineHeight, 0.2f, 0.2f, colorWhite, cgs.teamChatMsgs[i % chatHeight], 0, 0, 0, &cgs.media.font3 );
		}
	}
}
...
//	CG_FillRect( 0, 0, 640, 480, colorBlack );
	...
	// update the map and count every 1 second
	if ( (cg.mapVoteListTime + 1000) < cg.time && cgs.dbMode == 0 ) {
		cg.mapVoteListTime = cg.time;
...
{
	if( key == K_MOUSE1 ) {
		int pos	= (( cgs.cursorY - DB_MAPVOTE_Y ) / 12) + cgs.dbMapVoteListOffset;
		if( pos < 0 || pos >= cgs.dbNumMaps ) {
			return qfalse;
		}
		CG_MapVote_SetSelectedMap( pos );
...
void CG_MapVoteList_Draw( panel_button_t* button )
{
	...
	y2 += 15;
	if ( cgs.mapVoteMapY > 0 && cgs.mapVoteMapX + 1 >= cgs.mapVoteMapY ) {
		CG_Text_Paint_Ext( 290, y2, button->font->scalex, 
					button->font->scaley, button->font->colour, 
					va("^1XP Resetting!"), 
					0, 0, 0, button->font->font );
	...
	for( i = 0; i + cgs.dbMapVoteListOffset < cgs.dbNumMaps && i < 19; i++ ) {
		...
		if( cgs.dbSelectedMap == i + cgs.dbMapVoteListOffset ) {
			...
			pic = trap_R_RegisterShaderNoMip( va( "levelshots/%s.tga", 
						cgs.dbMaps[i + cgs.dbMapVoteListOffset]));
			if ( pic ) {
				CG_DrawPic( 620 - 96, 50, 96, 72, pic);
			}
			pic = trap_R_RegisterShaderNoMip( va( "levelshots/%s_cc.tga", 
						cgs.dbMaps[i + cgs.dbMapVoteListOffset]));
			if ( pic ) {
				CG_DrawPic( 620 - 192, 326+30-192-10, 192, 192, pic);
			}
			CG_Text_Paint_Ext( 290, y2, button->font->scalex, 
					button->font->scaley, button->font->colour, 
					va("Last Played            : %s",
						(cgs.dbMapLastPlayed[i+cgs.dbMapVoteListOffset] == -1 ?
						"^1Never" : va("^3%d ^7maps ago",
							cgs.dbMapLastPlayed[i+cgs.dbMapVoteListOffset]))),
					0, 0, 0, button->font->font );
			y2 += 15;
			CG_Text_Paint_Ext( 290, y2, button->font->scalex, 
					button->font->scaley, button->font->colour,
					va("Total Accumulated Votes: ^3%d", 
						cgs.dbMapTotalVotes[i+cgs.dbMapVoteListOffset]),
					0, 0, 0, button->font->font );
		}
		CG_Text_Paint_Ext( DB_MAPNAME_X, y, button->font->scalex, 
				button->font->scaley, button->font->colour, 
				cgs.dbMapDispName[i + cgs.dbMapVoteListOffset], 
				0, 30, 0, button->font->font );
		CG_Text_Paint_Ext( DB_MAPVOTE_X + 10, y, button->font->scalex,
				button->font->scaley, button->font->colour,
				va("%s%d", 
					(cgs.dbMapVotes[i+cgs.dbMapVoteListOffset] == 0?"":"^7"),
...
qboolean CG_Debriefing_Scrollbar_KeyDown( panel_button_t* button, int key ) {
	if( key == K_MOUSE1 ) {
		...
		// Mouse scroll down
		int ofs;
		if( CG_Debriefing_ScrollGetCount( button ) ) {
			ofs = CG_Debriefing_ScrollGetOffset( button );
...
{
	if( mapNum != cgs.dbSelectedMap ) {
		cgs.dbSelectedMap = mapNum;
	}
}
...
void CG_PanelButtonsRender_Button_Ext( rectDef_t* r, const char* text ) {
	...
	if( text ) {
		float w = CG_Text_Width_Ext( text, 0.2f, 0, &cgs.media.font3 );
		CG_Text_Paint_Ext( r->x + ((r->w+2) - w) * 0.5f, r->y + 11, 0.19f, 0.19f, hilight ? clrTxt_hi : clrTxt, text, 0, 0, 0, &cgs.media.font3 );
	}
}
...
void CG_PanelButtonsRender_Window_Ext( rectDef_t* r, const char* text, int align, int innerheight, float fontscale, int yofs ) {
	...
	if( text ) {
		...
		if( align == ITEM_ALIGN_CENTER ) {
			float w = CG_Text_Width_Ext( text, fontscale, 0, &cgs.media.font1 );
			x = r->x + (r->w - w) * 0.5f;
		...
		CG_Text_Paint_Ext( x, r->y + yofs, fontscale, fontscale, clrTxtBck, text, 0, 0, 0, &cgs.media.font1 );
	}
}
...
const char* CG_Debriefing2_WinStringForTeam( team_t team ) {
	switch( team ) {
		case TEAM_ALLIES:
			return "ALLIES WIN!";
		case TEAM_AXIS:
...
void CG_Debriefing2_Title_Draw( panel_button_t* button ) {
	...
	if( cg_gameType.integer == GT_WOLF_STOPWATCH ) {
		...
		CG_PanelButtonsRender_Window_Ext( &button->rect, CG_Debriefing2_WinStringForTeam(CG_Debriefing_FindWinningTeamForMap()), 0, 18, 0.25f, 16 );
		
		s = va( "CAMPAIGN STATUS: %s", CG_Debriefing2_WinStringForTeam(CG_Debriefing_FindOveralWinningTeam()) );
		w = CG_Text_Width_Ext( s, 0.25f, 0, &cgs.media.font1 );
		CG_Text_Paint_Ext( button->rect.x + (button->rect.w-w)*0.5f, button->rect.y + 16, 0.25f, 0.25f, clrTxtBck, s, 0, 0, 0, &cgs.media.font1 );
	} else {
		CG_PanelButtonsRender_Window_Ext( &button->rect, CG_Debriefing2_WinStringForTeam(CG_Debriefing_FindOveralWinningTeam()), 0, 18, 0.25f, 16 );
	}
	...
	CG_Text_Paint_Ext( x, button->rect.y + 16, 0.25f, 0.25f, clrTxtBck, s, 0, 0, 0, &cgs.media.font1 );
}

void CG_Debriefing2_MissionTitle_Draw( panel_button_t* button ) {

	if( cg_gameType.integer == GT_WOLF_CAMPAIGN && cgs.campaignInfoLoaded ) {
		CG_PanelButtonsRender_Window_Ext( &button->rect, va( "Campaign: %s", cgs.campaignData.campaignName ), 0, 12, 0.19f, 11 );
	} else if( cgs.arenaInfoLoaded ) {
		CG_PanelButtonsRender_Window_Ext( &button->rect, va( "Map: %s", cgs.arenaData.longname ), 0, 12, 0.19f, 11 );
	} else {
		CG_PanelButtonsRender_Window( button );
	}
}
...
void CG_Debriefing2_Awards_Parse( void ) {
	int i = 0;
...
void CG_Debriefing2_Awards_Draw( panel_button_t* button ) {
	...
	for( i = 0; i < NUM_ENDGAME_AWARDS; i++ ) {
		...
		CG_Text_Paint_Ext( button->rect.x + 28, y + 11, 0.19f, 0.19f, clrTxtBck, awardNames[ i ], 0, 0, 0, &cgs.media.font3 );

		CG_Text_Paint_Ext( button->rect.x + 28 + 180, y + 11, 0.19f, 0.19f, clrTxtBck, cgs.dbAwardNames[ i ], 0, 0, 0, &cgs.media.font3 );
		y += 16;
	}
}
...
void CG_Debriefing2_Maps_Draw( panel_button_t* button ) {
	...
	if( cg_gameType.integer == GT_WOLF_CAMPAIGN ) {
		...
		CG_Text_Paint_Ext( button->rect.x + 4, button->rect.y + 11, 0.19f, 0.19f, clrTxtBck, va( "Campaign: %s", cgs.campaignData.campaignName ), 0, 0, 0, &cgs.media.font3 );
		...
		for( i = 0; i < cgs.campaignData.mapCount; i++ ) {
			...
			CG_Text_Paint_Ext( button->rect.x + 8, y + 11, 0.19f, 0.19f, clrTxtBck, va( "%i. %s", i+1, cgs.campaignData.arenas[i].longname ), 0, 0, 0, &cgs.media.font3 );
			
			if( i <= cgs.currentCampaignMap ) {
				str = CG_Debriefing2_WinStringForTeam( CG_Debriefing_FindWinningTeamForPos( i + 1 ) );
				
				w = CG_Text_Width_Ext( str, 0.2f, 0, &cgs.media.font3 );
				CG_Text_Paint_Ext( button->rect.x + button->rect.w - w - 8, y + 11, 0.19f, 0.19f, clrTxtBck, str, 0, 0, 0, &cgs.media.font3 );
			}
...
void CG_Debriefing2_Mission_Draw( panel_button_t* button ) {
	if( cg_gameType.integer == GT_WOLF_CAMPAIGN ) {
		...
		if( cgs.campaignData.mapTC[0][0] && cgs.campaignData.mapTC[1][0] ) {
			...
			for( i = cgs.campaignData.mapCount-1; i >= 0; i-- ) {
				...
				w = CG_Text_Width_Ext( cgs.campaignData.arenas[i].longname, 0.2f, 0, &cgs.media.font3 );
				...
				if( x + 14 + w > button->rect.x + button->rect.w ) {
					...
					CG_Text_Paint_Ext( x - w - 15, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0, &cgs.media.font3 );
				} else {
					// x + pinhwidth (12) + pin right margin (0) => x + 12
					CG_Text_Paint_Ext( x + 12, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0, &cgs.media.font3 );
				} // b049
			}
	...
	if( cgs.arenaData.mappos[0] && cgs.arenaData.mappos[1] ) {
		...
		w = CG_Text_Width_Ext( cgs.arenaData.longname, 0.2f, 0, &cgs.media.font3 );
		...
		if( x + 14 + w > button->rect.x + button->rect.w ) {
			...
			// Width = pinhwidht (12) + pin right margin (0) + w + text margin (2) = 14 + w
			CG_FillRect( x + 1, y - 6 + 1, 14 + w, 12, colourFadedBlack );
			CG_FillRect( x, y - 6, 14 + w, 12, colorBlack );
		} // b049
		...
		if( x + 14 + w > button->rect.x + button->rect.w ) {
			// x - pinhwidth (12) - pin left margin (3) - w => x - w - 15
			CG_Text_Paint_Ext( x - w - 15, y + 3, 0.2f, 0.2f, colorWhite, cgs.arenaData.longname, 0, 0, 0, &cgs.media.font3 );
		} else {
			// x + pinhwidth (12) + pin right margin (0) => x + 12
			CG_Text_Paint_Ext( x + 12, y + 3, 0.2f, 0.2f, colorWhite, cgs.arenaData.longname, 0, 0, 0, &cgs.media.font3 );
		} // b049
	} else {
		CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, trap_R_RegisterShaderNoMip( "menu/art/unknownmap" ) );
	}
...
void CG_Debriefing2TeamSkillHeaders_Draw( panel_button_t* button ) {
	...
	for( j = 0; j < 2; j++ ) {
		for( i = 0; i <= SK_NUM_SKILLS; i++ ) {
			...
			if( *str ) {
				w = CG_Text_Width_Ext( str, 0.175f, 0, &cgs.media.font3 );

				//forty - campaign window
				CG_Text_Paint_Ext( button->rect.x + 60 + skillPositions[ i ] - (w*0.5f), button->rect.y + (j*11), 0.2f, 0.2f, clrTxtBck, str, 0, 0, 0, &cgs.media.font3 );
			}
		}
	}
}
...
void CG_Debriefing2TeamSkillXP_Draw( panel_button_t* button ) {
	...
	switch( team ) {
		case TEAM_AXIS:
			CG_Text_Paint_Ext( button->rect.x+3, button->rect.y + 11, scale, scale, clrTxtBck, "Axis", 0, 0, 0, &cgs.media.font3 );
			break;
		default:
	...
	for( i = 0; i <= SK_NUM_SKILLS; i++ ) {
		...
		w = CG_Text_Width_Ext( str, scale, 0, &cgs.media.font3 );
		
		// forty - campaign window
		CG_Text_Paint_Ext( button->rect.x + 60 + skillPositions[ i ] - (w*0.5f), button->rect.y + 11, scale, scale, clrTxtBck, str, 0, 0, 0, &cgs.media.font3 );
	}
}
