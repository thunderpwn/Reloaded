...
static void CG_Obituary( entityState_t *ent ) {
	...
	if( attacker == target ) {
		...
		case MOD_REFLECTED_FF: // perro
			message = "was killed by his own friendly fire.";
		break;
	...
	if (message) {
		...
		finalMsg = va("%s %s.", targetName, message);
	...
	if( attacker == cg.snap->ps.clientNum ) {
		...
		CG_PriorityCenterPrint( s, CP_DEFAULTHEIGHT, BIGCHAR_WIDTH * 0.6, 1 );
