...
//extern const char* bg_skillRewards[SK_NUM_SKILLS][NUM_SKILL_LEVELS-1];
...
void CG_NewClientInfo( int clientNum ) {
	...
	if( clientNum == cg.clientNum ) {
		...
		/*if(!cgs.limboLoadoutSelected) {
			...
			cgs.clientinfo[ cg.clientNum ].weapon = newInfo.weapon;
			cgs.clientinfo[ cg.clientNum ].latchedweapon = newInfo.latchedweapon;
			cgs.clientinfo[ cg.clientNum ].secondaryweapon = newInfo.secondaryweapon;

			for( i = 0; i < 3; i++ ) {
				if( teamOrder[i] == newInfo.team ) {
					cgs.ccSelectedTeam = i;
				}
			}

			// kw: we need the class to select the proper weapons.
			if( ci->team != TEAM_SPECTATOR ) {			
				cgs.ccSelectedClass = newInfo.cls;
			}

			cgs.ccSelectedWeapon = CG_LimboPanel_WeaponFromNum(newInfo.weapon);

			//FIXME make sure this doesn't break akimbo/pistol picking.
		...
		if( newInfo.refStatus != ci->refStatus ) {
			if( newInfo.refStatus <= RL_NONE ) {
				...
				CG_Printf("^3*** You have been stripped of your referee status! ***\n");

			} else {
				trap_Cvar_Set( "cg_ui_voteFlags", "0");
				CG_Printf("^2*** You have been authorized \"%s\" status ***\n", ((newInfo.refStatus == RL_RCON) ? "rcon" : "referee"));
	...
	*ci = newInfo;
	...
	if( !ci->character ) {
		ci->character = BG_GetCharacter( ci->team, ci->cls, ci->characterType );
	}
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if( (cent->currentState.eFlags & EF_DEAD)
		...
		// kw: fixed revive icons showing when following non-medics as spectator
...
animation_t *CG_GetLimboAnimation( playerInfo_t *pi, const char *name ) {
	int				i;
	bg_character_t	*character = BG_GetCharacter( pi->teamNum, pi->classNum, cgs.clientinfo[cg.clientNum].characterType );
