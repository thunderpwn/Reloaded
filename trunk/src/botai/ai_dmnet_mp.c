...
int AINode_MP_Seek_ActivateEntity(bot_state_t *bs) {
	...
	if (BotInLava(bs))	bs->tfl |= TFL_LAVA;
	if (BotInSlime(bs))	bs->tfl |= TFL_SLIME;
...
int AINode_MP_Seek_NBG(bot_state_t *bs)
{
	...
	//if waiting for something
		...
		// check for enemies
		if (bs->enemy < 0) BotFindEnemyMP(bs, -1, qfalse);
		if (bs->enemy >= 0) {
...
int AINode_MP_AvoidDanger(bot_state_t *bs)
{
	...
	if (bs->flags & BFL_MISCFLAG) {
		...
		if (level.time%2000 < 1000) trap_EA_MoveLeft( bs->client );
	...
	if (move) {
		...
		if (bs->enemy >= 0) {
			...
			if (goal.entitynum > -1 && goal.entitynum < level.maxclients && BotCarryingFlag( goal.entitynum )) {
				...
				if (!flaggoal) flaggoal = BotFindNextStaticEntity(NULL, BOTSTATICENTITY_FLAGONLY_MULTIPLE);
				if (flaggoal) {
...
*/	
	...
	if (fabs(target[2] - bs->origin[2]) < 80) target[2] = bs->origin[2];
				...
				(numList = BotNumTeamMatesWithTarget( bs, bs->client, list, MAX_CLIENTS )))
			{
				...
				for (i=0; i<numList; i++) {
					...
					if ((VectorDistanceSquared( tbs->origin, tbs->defendgoal.origin ) < SQR(1500)) &&
						...
						if (t > 300) t = 300;
						if (t > 100) bs->stand_time = level.time + 1000 + 5*t;
					}
				}
			}
		}
...
int AINode_MP_DynamiteTarget(bot_state_t *bs)
{
	...
	if (goalDist < SQR(32)) {
		...
		if (!trav) {
			...
			vectoangles( dir, bs->ideal_viewangles );
			// hold fire
			if (bs->cur_ps.weapon == WP_PLIERS) trap_EA_Attack( bs->client );
			return qtrue;
...
int AINode_MP_PlantMine(bot_state_t *bs)
{
	...
	if(VectorDistanceSquared( bs->origin, goal.origin ) < SQR(32)) {
		...
		if (!trav) {
			...
			trap_EA_SelectWeapon(bs->client, bs->weaponnum);
			// hold fire
			if (!VectorLengthSquared(bs->velocity) && fabs(bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 && bs->cur_ps.weapon == bs->weaponnum && !bs->cur_ps.grenadeTimeLeft) trap_EA_Attack( bs->client );
		} else if (trav->s.teamNum >= 4) {	// the dynamite is not armed
			// switch to pliers and arm the dynamite
			bs->weaponnum = WP_PLIERS;
			trap_EA_SelectWeapon(bs->client, bs->weaponnum);
...
int AINode_MP_Battle_Chase(bot_state_t *bs)
{
	...
	if ((bs->weaponnum != WP_KNIFE) && trap_BotTouchingGoal(bs->origin, &goal)) bs->chase_time = 0;
...
void AIEnter_MP_MoveToAutonomyRange(bot_state_t *bs)
{
	...
	//
	if (!BotGetMovementAutonomyPos( bs, pos )) {
		if (g_developer.integer) G_Printf( "AIEnter_MP_MoveToAutonomyRange: autonomy pos unknown\n" );
	}
