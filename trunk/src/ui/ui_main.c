...
// tjw: big ugly global string used as a buffer to prevent alloc slowdowns
char bigTextBuffer[30000] = {""};
...
void _UI_Refresh( int realtime )
{
	...
	// tjw: update the font selections if the user clicks on the
	//      "DEFAULT" button.
	if(uiInfo.fontDefaults) {
		UI_MenuSelectFonts();
		uiInfo.fontDefaults = qfalse;
	}
		...
		if( uiInfo.campaignList[map].mapTC[0][0] && uiInfo.campaignList[map].mapTC[1][0] ) {
			...
			for( i = 0; i < uiInfo.campaignList[map].mapCount; i++ ) {
				...
				if( x + 10 + w > rect->x + rect->w ) {
					...
					// Width = pinhwidht (8) + pin right margin (0) + w + text margin (2) = 10 + w
...
static void UI_LoadFonts(void)
{
	...
	uiInfo.fontCount = 0;
	numdirs = trap_FS_GetFileList("fonts", "dat", 
		bigTextBuffer, sizeof(bigTextBuffer) );
	dirptr  = bigTextBuffer;
	for( i = 0; i < numdirs; i++ ) {
		dirlen = strlen( dirptr ) + 1;
		uiInfo.fontList[uiInfo.fontCount] = String_Alloc(dirptr);
		dirptr += dirlen;
		uiInfo.fontCount++;
		if (uiInfo.fontCount >= MAX_FONTS) {
			break;
		}
	}
}
...
static void UI_LoadDemos() {
	...
	uiInfo.demoCount = trap_FS_GetFileList( "demos", demoExt,
		bigTextBuffer, sizeof(bigTextBuffer) );
...
char *UI_FontName(const char *filename) 
{
...
int UI_FontSize(const char *filename) 
{
	char size[4] = {""};
	int i = 0;
...
void UI_MenuSelectFonts(void)
{
	int i;

	for(i = 0; i < uiInfo.fontCount; i++) {
...
void UI_RunMenuScript(char **args) {
	...
	if (String_Parse(args, &name)) {
		...
		if (Q_stricmp(name, "StartServer") == 0) {
			...
			int i,index = 0;
			// kw: fixes changing selected map when jumping to other menu's and back.
			for( i = 0; i < uiInfo.mapCount && i != ui_currentNetMap.integer; i++) {
				if (uiInfo.mapList[i].active || (ui_netGameType.integer == GT_WOLF_CAMPAIGN && uiInfo.mapList[i].typeBits & (1<<GT_WOLF)) )
					index++;
			}
			Menu_SetFeederSelection(NULL, FEEDER_ALLMAPS, index, NULL);		
...
//			UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);
			...
			trap_Cvar_Set( "ui_netSource", va("%i", AS_GLOBAL) );
			...
			trap_Cvar_Set( "ui_browserShowPunkBuster", "0" );
			...
			trap_Cvar_Set( "ui_browserShowModName", "" );
			trap_Cvar_Set( "ui_browserFilterHost", "" );
...
//			Menus_ActivateByName( "background_1", qtrue );
		...
		} else if (Q_stricmp(name, "votePutSpec") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				trap_Cmd_ExecuteText( EXEC_APPEND, va("callvote putspec \"%s\"\n", uiInfo.playerNames[uiInfo.playerIndex]) );
			}
		...
		} else if (!Q_stricmp(name, "votePoll")) {
			char poll[200];
			trap_Cvar_VariableStringBuffer("ui_poll",
				poll, sizeof(poll));
			trap_Cmd_ExecuteText(EXEC_APPEND,
				va("callvote poll \"%s\"\n", poll));
		} else if (!Q_stricmp(name, "refPoll")) {
			char poll[200];
			trap_Cvar_VariableStringBuffer("ui_poll",
				poll, sizeof(poll));
			trap_Cmd_ExecuteText(EXEC_APPEND,
...
//			int i = 0;
		...
		else if(!Q_stricmpn(name, "ui_loadfont", 11)) {
			char *fname;
			int fsize;
			int fnum;
			
			fname = UI_FontName(uiInfo.fontList[uiInfo.fontIndex]);
			fsize = UI_FontSize(uiInfo.fontList[uiInfo.fontIndex]);
			sscanf(name, "ui_loadfont%i", &fnum);
			UI_LoadFont(fnum, fname, fsize);
		}
		else if(!Q_stricmpn(name, "cg_loadfont", 11)) {
			char *fname;
			int fsize;
			int fnum;
			...
			fname = UI_FontName(uiInfo.fontList[uiInfo.fontIndex]);
			fsize = UI_FontSize(uiInfo.fontList[uiInfo.fontIndex]);
			sscanf(name, "cg_loadfont%i", &fnum);
			trap_Cmd_ExecuteText(EXEC_APPEND,
				va("loadfont %i %s %i\n", fnum, fname, fsize));
		}
		else if(!Q_stricmp(name, "defaultfonts")) {
			trap_Cmd_ExecuteText(EXEC_APPEND,
				"loadfont defaults\n");
			trap_Cmd_ExecuteText(EXEC_APPEND,
				"ui_loadfont defaults\n");
			// tjw: this is hackish.  Is there a way to tell the
			//      engine to just load up the default values for
			//      a cvar?
			trap_Cvar_Set("cg_fontQAScale", "0.2");
			trap_Cvar_Set("cg_fontLimboScale", "0.2");
			trap_Cvar_Set("cg_fontOIDScale", "0.2");
			trap_Cvar_Set("cg_fontCPScale", "0.2");
			trap_Cvar_Set("cg_fontBPScale", "0.25");
		}
		else {
...
static void UI_BuildServerDisplayList(qboolean force) {
	...
	for (i = 0; i < count; i++) {
		...
		if (ping >/*=*/ 0 || ui_netSource.integer == AS_FAVORITES) {
			...
			// kw: ADD NEW FILTERS HERE

			// kw: MOD filter
			trap_Cvar_Update( &ui_browserShowModName );
			if ( *ui_browserShowModName.string ) {
				if (Q_stricmp(Info_ValueForKey(info, "game"), ui_browserShowModName.string) ) {
					trap_LAN_MarkServerVisible(ui_netSource.integer, i, qfalse);
					continue;
				}
			}
			
			// kw: ServerName filter
			if ( strlen(uiInfo.browserFilterHost) > 0 ) {
				if( !stristr( Q_CleanStr(Info_ValueForKey(info, "hostname")), uiInfo.browserFilterHost) ) {	
					trap_LAN_MarkServerVisible(ui_netSource.integer, i, qfalse);
					continue;
				}
			}
...
static int UI_FeederCount(float feederID) {
	...
	else if(feederID == FEEDER_FONTS) {
		return uiInfo.fontCount;
	}
	return 0;
}
...
static const char *UI_SelectedMap(qboolean singlePlayer, int index, int *actual) {
	...
	*actual = 0;
	...
	if( game == GT_WOLF_CAMPAIGN ) {
		...
		for (i = 0; i < uiInfo.mapCount; i++) {
			if (uiInfo.mapList[i].active && uiInfo.mapList[i].typeBits & (1<<game)) {
				if (c == index) {
					*actual = i;
					return uiInfo.mapList[i].mapName;
				} else {
...
const char *UI_FeederItemText( float feederID, int index, int column, qhandle_t *handles, int *numhandles ) {
	...
	*numhandles = 0;
	if (feederID == FEEDER_HEADS) {
		...
		if(index >=0 && index < uiInfo.fontCount) {
			return va("%s (%ipt)",
				UI_FontName(uiInfo.fontList[index]),
				UI_FontSize(uiInfo.fontList[index]));
		}
	}
	// -NERVE - SMF
	return "";
    ...
    if (index >= 0 && index < uiInfo.q3HeadCount) {
      ...
      trap_Cvar_Set( "headmodel", uiInfo.q3HeadNames[index]);
		...
		map = (feederID == FEEDER_ALLMAPS) 
						? ui_currentNetMap.integer 
						: ui_currentMap.integer;
		game = (feederID == FEEDER_ALLMAPS) 
						? ui_netGameType.integer
...
static void UI_BuildQ3Model_List( void )
{
	...
	for (i=0; i<numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		for (j=0; j<numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS;j++,fileptr+=filelen+1)
		{
			...
			COM_StripExtensionSafe(fileptr,
				skinname, sizeof(skinname));
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
  ...
  if (Menu_Count() > 0) {
	  ...
	  case UIMENU_INGAME:
		...
		case UIMENU_WM_SPAWNALT:
			uiInfo.uiDC.cursorx = 639;
			uiInfo.uiDC.cursory = 479;
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			Menus_OpenByName( "wm_spawnpointalt" );
			return;

		case UIMENU_HUDEDITOR_SAVE:
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_OpenByName( "ingame_hudeditor_save" );
			return;

		case UIMENU_HUDEDITOR_LOAD:
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_OpenByName( "ingame_hudeditor_load" );
			return;
...
// kw: for playonline browser filters
vmCvar_t	ui_browserShowModName;
vmCvar_t	ui_browserFilterHost;
...
vmCvar_t ui_font3;
vmCvar_t ui_font4;
vmCvar_t ui_font1size;
vmCvar_t ui_font2size;
vmCvar_t ui_font3size;
vmCvar_t ui_font4size;
vmCvar_t cg_font1;
vmCvar_t cg_font2;
vmCvar_t cg_font3;
vmCvar_t cg_font4;
...
//	{ &ui_actualNetGameType, "ui_actualNetGametype", "5", CVAR_ARCHIVE },		// NERVE - SMF - hardwired for now
	...
	// kw: for playonline browser filters
	{ &ui_browserShowModName,	"ui_browserShowModName",		"",		CVAR_ARCHIVE },
	{ &ui_browserFilterHost,		"ui_browserFilterHost",		"",		CVAR_ARCHIVE },
	// kw: menu refresh/stoprefresh toggle
	{ NULL,					"ui_serverlistRefresh",		"0",		CVAR_ROM },
	//
	{ &ui_font1, "ui_font1", "ariblk", CVAR_ARCHIVE },
	{ &ui_font2, "ui_font2", "ariblk", CVAR_ARCHIVE },
	{ &ui_font3, "ui_font3", "courbd", CVAR_ARCHIVE },
	{ &ui_font4, "ui_font4", "courbd", CVAR_ARCHIVE },
	{ &ui_font1size, "ui_font1size", "16", CVAR_ARCHIVE },
	{ &ui_font2size, "ui_font2size", "27", CVAR_ARCHIVE },
	{ &ui_font3size, "ui_font3size", "21", CVAR_ARCHIVE },
	{ &ui_font4size, "ui_font4size", "30", CVAR_ARCHIVE },
...
static void UI_StopServerRefresh( void )
{
	...
	trap_Cvar_Set( "ui_serverlistRefresh", "0");
...
static void UI_StartServerRefresh( qboolean full )
{
	...
	// kw: 	temporarly store the browserFilterHost cvar here,
	//		so it doesn't mess up the list if it's changed while generating the list.
	trap_Cvar_Update( &ui_browserFilterHost);
	Com_sprintf( uiInfo.browserFilterHost, sizeof(uiInfo.browserFilterHost), ui_browserFilterHost.string);
	...
	trap_Cvar_Set( "ui_serverlistRefresh", "1");
...
qboolean UI_LoadFont(int num, char *name, int size)
{
	int len = 0;
	fileHandle_t f;
	...
	switch(num) {
	case 1:
		trap_R_RegisterFont(name, size,
			&uiInfo.uiDC.Assets.fonts[0]);
		trap_Cvar_Set("ui_font1", name);
		trap_Cvar_Set("ui_font1size", va("%i", size));
		break;
	case 2:
		trap_R_RegisterFont(name, size,
			&uiInfo.uiDC.Assets.fonts[1]);
		trap_R_RegisterFont(name, size,
			&bg_loadscreenfont1);
		trap_Cvar_Set("ui_font2", name);
		trap_Cvar_Set("ui_font2size", va("%i", size));
		break;
	case 3:
		trap_R_RegisterFont(name, size,
			&uiInfo.uiDC.Assets.fonts[2]);
		trap_Cvar_Set("ui_font3", name);
		trap_Cvar_Set("ui_font3size", va("%i", size));
		break;
	case 4:
		trap_R_RegisterFont(name, size,
			&uiInfo.uiDC.Assets.fonts[3]);
		trap_R_RegisterFont(name, size,
			&bg_loadscreenfont2);
		trap_Cvar_Set("ui_font4", name);
		trap_Cvar_Set("ui_font4size", va("%i", size));
		break;
	default:
		return qfalse;
	}
