...
//vmCvar_t		url; //Dens: see comments below
...
vmCvar_t g_privateMessageSound;
vmCvar_t g_XPSave;
...
vmCvar_t g_ATB_offtime; // ATB off after offtime minutes
vmCvar_t g_logAdmin;
...
vmCvar_t g_maxXP;
vmCvar_t g_maxXPResetWarn; // gabriel: Warn user when XP will be reset
...
// gabriel - bot admin
vmCvar_t g_bot_maxXP;
vmCvar_t g_bot_minPlayers;
...
vmCvar_t g_spreeOptions;
vmCvar_t g_multikillTime;

vmCvar_t g_spoofOptions;

// Dens: advanced warning system
vmCvar_t g_warningOptions;
vmCvar_t g_maxWarnings;
vmCvar_t g_warningDecay;

// Dens: max namechanges per map
vmCvar_t g_maxNameChanges;
// Dens: maximum amount of panzersuicides, before shooting medicpacks
vmCvar_t g_maxPanzerSuicides;
vmCvar_t g_panzerPackDistance;
// Chaos: maximum amount of selfkills
vmCvar_t g_maxSelfkills;
// Chaos: define who sees the result of a vote
vmCvar_t g_voteResultsMinLevel;
// Chaos: Set the time you have to wait between using custom ! commands
vmCvar_t g_minCommandWaitTime;
...
vmCvar_t g_characterModsEnabled;
vmCvar_t g_memberLevel;
vmCvar_t g_adminLevel;

vmCvar_t g_unlockDropWeapons;
...
//	{ &g_debugBullets, "g_debugBullets", "0", CVAR_CHEAT, 0, qfalse},	//----(SA)	added
	...
	{ &g_antilagDelay, "g_antilagDelay", "0", CVAR_SERVERINFO | CVAR_ARCHIVE, 0, qfalse },
...
//	{ &g_movespeed, "g_movespeed", "127", CVAR_CHEAT, 0, qfalse },
	...
	// Dens: I see no reason why we should define this CVAR in the gamecode.
	// It isn't used in G_CvarUpdateSafe and if admins would really want it,
	// they can use the sets command.
	...
	{ &g_debugCharacters,	"g_debugCharacters", "0", 0		},
	...
	{ &g_privateMessageSound, "g_privateMessageSound", "", 0 },
	{ &g_XPSave, "g_XPSave", "0", 0 },
	{ &g_XPSaveFile, "g_XPSaveFile", "xpsave.cfg", 0 },
	{ &g_XPSaveMaxAge_xp, "g_XPSaveMaxAge_xp", "1d", 0 },
	{ &g_XPSaveMaxAge, "g_XPSaveMaxAge", "1w", 0 },
	...
	{ &g_ATB_offtime, "g_ATB_offtime", "0", 0 },
	...
	{ &g_medicSelfhealTime, "g_medicSelfhealTime", "0", 0 },
	...
	{ &g_censorNeil, "g_censorNeil", "0", 0 },
	{ &g_censorNamesNeil, "g_censorNamesNeil", "0", 0 },
	{ &g_killRating, "g_killRating", "3", 0 },
	{ &g_trackBehavior, "g_trackBehavior", "0", 0 },
...
// unlagged server options
	...
	{ &sv_privateClients, "sv_privateClients", "0", CVAR_SYSTEMINFO, 0, qfalse },
	...
	// gabriel - bot admin
	{ &g_bot_maxXP, "g_bot_maxXP", "-1", 0},
	{ &g_bot_minPlayers, "g_bot_minPlayers", "-1", 0},
	...
	{ &g_damageXP, "g_damageXP", "0", CVAR_SERVERINFO },
	{ &g_damageXPLevel, "g_damageXPLevel", "50", CVAR_SERVERINFO },

	// forty - mine id
	{ &g_mineid, "g_mineid", "0", 0 },

	// forty - #127 censor xp penalty
	{ &g_censorXP, "g_censorXP", "5", 0 },

	// forty - #377 Drop Message
	{ &g_dropMsg, "g_dropMsg", "", 0 },

	// forty - in mod flood protection
	{ &g_floodprotect, "g_floodprotect", "1", 0 },
	{ &g_floodthreshold, "g_floodthreshold", "6", 0 },
	{ &g_floodWait, "g_floodWait", "1000", 0 },
	...
	// Dens: position of greetings
	{ &g_greetingPos, "g_greetingPos", "0", 0 },

	// Dens: time between two votes
	{ &g_noVoteTime, "g_noVoteTime", "0", 0 },

	// Dens: settings file
	{ &g_settings, "g_settings", "", 0, 0 },
	...
	{ &g_maxWarnings, "g_maxWarnings", "3", 0},
	{ &g_warningDecay, "g_warningDecay", "24", 0},

	{ &g_maxNameChanges, "g_maxNameChanges", "3", 0},
	{ &g_maxPanzerSuicides, "g_maxPanzerSuicides", "-1", 0},
	{ &g_panzerPackDistance, "g_panzerPackDistance", "0", 0 },
	{ &g_maxSelfkills, "g_maxSelfkills", "-1", 0},
	{ &g_voteResultsMinLevel, "g_voteResultsMinLevel", "-1", 0},
	...
	{ &g_characterModsEnabled, "g_characterModsEnabled", "1", CVAR_SERVERINFO},
	{ &g_memberLevel, "g_memberLevel", "0", 0},
	{ &g_adminLevel, "g_adminLevel", "0", 0},
	{ &g_unlockDropWeapons, "g_unlockDropWeapons", "1", 0},
...
void G_CheckForCursorHints( gentity_t *ent ) {
	...
	// CHRUKER: b080 - Breakable damage indicator can wrap when the entity
	//          has a lot of health
			...
			switch (checkEnt->s.eType) {
				case ET_CORPSE:
					if( !ent->client->ps.powerups[PW_BLUEFLAG] && !ent->client->ps.powerups[PW_REDFLAG] &&
						(!ent->client->ps.powerups[PW_OPS_DISGUISED] || g_coverts.integer & COVERT_STEAL_OTHER_UNIFORM)) {
...
void G_UpdateEtpubinfo(void)
{
	...
	Info_SetValueForKey(cs, "g_doubleJumpHeight",
		va("%f", g_doubleJumpHeight.value));
	Info_SetValueForKey(cs, "g_staminaRecharge",
		va("%f", g_staminaRecharge.value));
	Info_SetValueForKey(cs, "g_weapons",
		va("%i", g_weapons.integer));
	Info_SetValueForKey(cs, "g_coverts",
		va("%i", g_coverts.integer));
	Info_SetValueForKey(cs, "g_medics",
		va("%i", g_medics.integer));
	Info_SetValueForKey(cs, "g_throwableKnives",
		va("%i", g_throwableKnives.integer));
	Info_SetValueForKey(cs, "g_playerRating",
		va("%i", g_playerRating.integer));
	Info_SetValueForKey(cs, "g_killRating",
		va("%i", g_killRating.integer));
	Info_SetValueForKey(cs, "mapX",
		va("%i",
			(level.mapsSinceLastXPReset >= g_resetXPMapCount.integer) ?
			0 : level.mapsSinceLastXPReset));
	Info_SetValueForKey(cs, "mapY",
		((g_XPSave.integer & XPSF_NR_EVER) ? "0" :
			va("%i", g_resetXPMapCount.integer)));

	Info_SetValueForKey(cs, "team_maxPanzers",
		va("%s", team_maxPanzers.string));
	Info_SetValueForKey(cs, "team_maxMortars",
		va("%s", team_maxMortars.string));
	Info_SetValueForKey(cs, "team_maxFlamers",
		va("%s", team_maxFlamers.string));
	Info_SetValueForKey(cs, "team_maxMG42s",
		va("%s", team_maxMG42s.string));
	Info_SetValueForKey(cs, "team_maxGrenLaunchers",
		va("%s", team_maxGrenLaunchers.string));
	...
	Info_SetValueForKey(cs, "skill_medic",
		va("%s", g_skillMedic.string));
	Info_SetValueForKey(cs, "skill_engineer",
		va("%s", g_skillEngineer.string));
	Info_SetValueForKey(cs, "skill_fieldops",
		va("%s", g_skillFieldOps.string));
	Info_SetValueForKey(cs, "skill_covertops",
		va("%s", g_skillCovertOps.string));
	Info_SetValueForKey(cs, "skill_battlesense",
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	level.atb_has_run = qfalse;
	{
		...
		memcpy( &model, &level.win_probability_model,
				sizeof( win_probability_model_t ) );

		memset( &level, 0, sizeof( level ) );

		memcpy( &level.voteInfo, &votedata, sizeof( voteInfo_t ) );
		memcpy( &level.win_probability_model, &model,
				sizeof( win_probability_model_t ) );
...
//		BotAILoadMap( restart );
	...
	if(g_floodprotect.integer) {
		i = trap_Cvar_VariableIntegerValue("sv_floodprotect");
		if(i) {
			G_Printf("Disabling sv_floodprotect\n");
...
int QDECL SortRanks( const void *a, const void *b ) {
	...
	if( g_gametype.integer == GT_WOLF_LMS ) {
		...
		// josh: I don't think this will hurt anything. The only thing
		// that cares about the order of level.sortedClients is the
		// scoreboard itself, which is what I'm changing.
		if (g_playerRating.integer & PLAYER_RATING_SCOREBOARD) {
			totalXP[0] =
				1.0 / (1.0 + exp(-ca->sess.rating
				/sqrt(1.0+3.0*ca->sess.rating_variance*20.0
				/(M_PI*M_PI))));
			totalXP[1] =
				1.0/(1.0+exp(-cb->sess.rating
				/ sqrt(1.0+3.0*cb->sess.rating_variance
				* 20.0 / (M_PI*M_PI))));
		} else {
			for( totalXP[0] = totalXP[1] = 0, i = 0;
			    i < SK_NUM_SKILLS; i++ ) {
...
//	float			timeLived;
	...
	// initialize the map voting vars
...
void BeginIntermission( void ) {
	...
	// initialize and populate the map struct for map voting
	if ( g_gametype.integer == GT_WOLF_MAPVOTE ) {
		level.mapVoteNumMaps = trap_FS_GetFileList("maps", ".bsp",
					bspNames, sizeof(bspNames));
		bspptr = bspNames;
		...
		for ( i = 0; i < level.mapVoteNumMaps; i++, bspptr += len + 1 ) {
			len = strlen(bspptr);
			Q_strncpyz(level.mapvoteinfo[i].bspName,
					Q_StrReplace(bspptr,".bsp",""),
					sizeof(level.mapvoteinfo[0].bspName));
		...
		for ( i = 0; i < len; i++ ) {
			...
			// played too recently
			if ( level.mapvoteinfo[i].lastPlayed <=
						g_minMapAge.integer &&
						level.mapvoteinfo[i].lastPlayed != -1 ) {
				level.sortedMaps[i] = -1;
			...
			} else if ( strstr(g_excludedMaps.string,
						va(":%s:",level.mapvoteinfo[i].bspName)) ) {
				level.sortedMaps[i] = -1;
				level.mapvoteinfo[i].zOrder = 0;
				level.mapVoteNumMaps--;
				if ( level.mapvoteinfo[i].lastPlayed != -1 ) {
					level.mapvoteinfo[i].lastPlayed++;
				}
			}
		}

		qsort( level.sortedMaps, len,
			sizeof(level.sortedMaps[0]), G_SortMapsByzOrder );
	...
	if( g_gametype.integer == GT_WOLF_CAMPAIGN ) {
		...
		for ( i = 0; i < maxMaps; i++ ) {
			...
			if (curMapVotes > highMapVote ||
					...
					highMapAge = (curMapAge == -1 ? 9999 : curMapAge);
			}
		}
		if ( highMapVote > 0 ) {
...
int G_EndRoundCountdown()
{
	...
	// forty - ASD's fun_on_ice and jv-trainjump-v3 crash fix.
	if(!level.gameManager)
		return -1;
...
void LogExit( const char *string ) {
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		G_UpdateSkillTime(&g_entities[level.sortedClients[i]], qtrue);
	}

	//josh: Calculate playerratings
	//      Moved this up since I was getting bad GUIDs. Hope if helps.
...
void CheckIntermissionExit( void ) {
	...
	int i;
	int players = 0; // Dens: players are people in a team (Allied or Axis)
	// rain - for #105
	gclient_t *cl;
	int ready = 0, notReady = 0;
	int voted = 0, notVoted = 0;
	...
	if(!(fActions & EOM_MATCHINFO) && level.time - level.intermissiontime > 800) {
		...
		// pheno: display lastblood message
		G_LastBloodMessage();
		...
		if (g_spreeOptions.integer & SPREE_SHOW_ACTIVE_INTERMISSION) {
			G_Streakers(qtrue);
			G_MapLongStreak(qtrue);
		}
		if(g_spreeOptions.integer & SPREE_SHOW_RECORD_INTERMISSION){
			G_ShowSpreeRecord(qfalse);
		}
	}

	// Count ready players and count players who have voted on the next map
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		cl = level.clients + level.sortedClients[i];
		// Completely ignore people still loading and bots
		if ((cl->pers.connected != CON_CONNECTED) ||
	...
	// Dens: ref_allready should always skip intermission
    if(level.ref_allready){
        exit = qtrue;
    }else{
		// Dens: we don't want an empty server to stay in the same map
        if(players > 0) {
            ...
            if (((100.0f * (voted / (players * 1.0f))) <
                (g_intermissionReadyPercent.value * 1.0f)) &&
                (g_mapVoteFlags.integer & MAPVOTE_ALT_INTERMISSION) ) {

				return;
            }

            if((100.0f * (ready / (players * 1.0f))) >=
                (g_intermissionReadyPercent.value * 1.0f)) {
...
void CheckVote( qboolean forced ) {
	...
	if(!level.voteInfo.voteTime ||
		...
		(level.time - level.voteInfo.voteTime < 1000 && !forced)) {
	...
	if( level.voteInfo.voteYes > pcnt*total/100 ) {
		...
		if(level.voteInfo.voteYes > total + 1) {
			...
			if(g_voting.integer & VOTEF_DISP_NUMVOTES){
				if(forced){
					AP("cpm \"^5Vote passed by admin!\n\"");
				}else{
					AP(va("cpm \"^5Vote passed! YES: ^1%i ^5votes, NO: ^1%i ^5votes\n\"",
						level.voteInfo.voteYes, level.voteInfo.voteNo));
				}
			}else{
				AP("cpm \"^5Vote passed!\n\"");
		...
		if(g_voting.integer & VOTEF_DISP_NUMVOTES){
			if(forced){
				AP(va("cpm \"^2Vote CANCELED by admin! ^3(%s)\n\"", level.voteInfo.voteString));;
			}else{
				AP(va("cpm \"^2Vote FAILED! ^3(%s)  ^5YES: ^1%i ^5votes, NO: ^1%i ^5votes\n\"",
					level.voteInfo.voteString, level.voteInfo.voteYes, level.voteInfo.voteNo));
			}
		}else{
	...
	if(g_voteResultsMinLevel.integer > -1) {
		for( i = 0; i < level.numConnectedClients; i++ ) {
			loopent = g_entities + level.sortedClients[i];
			if(_shrubbot_level(loopent) >= g_voteResultsMinLevel.integer ||
				loopent->client->sess.referee == RL_REFEREE){
				trap_SendServerCommand( loopent-g_entities, va("cpm \"^2Axis vote results, yes: %i, no %i\n\"",
					level.voteInfo.axisVoteYes, level.voteInfo.axisVoteNo));
				trap_SendServerCommand( loopent-g_entities, va("cpm \"^2Allies vote results, yes: %i, no %i\n\"",
					level.voteInfo.alliesVoteYes, level.voteInfo.alliesVoteNo));
			}
		}
	}

	level.voteInfo.axisVoteYes = level.voteInfo.axisVoteNo =
		level.voteInfo.alliesVoteYes = level.voteInfo.alliesVoteNo = 0;
...
uebrgpiebrpgibqeripgubeqrpigubqifejbgipegbrtibgurepqgbn%i", level.time )
	...
	G_ActiveTeamBalance(qfalse, qfalse);

	G_UnevenTeamCheck();

	if(level.time >= level.nextBannerTime ) {
		G_displayBanner(0);
	}
	...
	// Perro: Display current streaks
	if ((g_spreeOptions.integer & SPREE_SHOW_ACTIVE_INGAME) &&
			((level.time % 1000) == 0) &&
			(((level.time / 1000) % 60) == 0))  {
		G_Streakers(qfalse);
	}
	...
	// gabriel: Check for adding/removing bots every 3 seconds
	if (level.time % 3000 == 0) {
		DoGBotMinPlayers();
	}

	// gabriel: Warn people who are about to lose their XP because of g_maxXP
	if (level.time % 30000 == 0) {
		if ((g_maxXP.integer > 0) && g_maxXPResetWarn.integer) {
			int warnThreshold = g_maxXPResetWarn.integer;
			int totalXP;

			if (strlen(g_maxXPResetWarn.string) > 0) {
				if (g_maxXPResetWarn.string[strlen(g_maxXPResetWarn.string) - 1] == '%') {
					warnThreshold = g_maxXP.integer * g_maxXPResetWarn.integer / 100;
				}
			}

			// If the threshold is negative, that means it is a difference from g_maxXP
			// (ie, if threshold is -50, then it really should be g_maxXP - 50)
			if (warnThreshold < 0)
				warnThreshold += g_maxXP.integer;
			...
			for( i = 0; i < level.numConnectedClients; i++ ) {
				...
				totalXP = 0;
				for( ii = 0; ii < SK_NUM_SKILLS; ii++ )
					totalXP += ent->client->sess.skillpoints[ii];

				if (warnThreshold <= totalXP)
					CP(va("cp \"^1WARNING: ^7Your XP will be reset when you reach ^1%i ^7XP!\" 1",
						g_maxXP.integer));
			}
		}
	}
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		if(g_spreeOptions.integer & SPREE_SHOW_KILLS &&
			...
			g_entities[level.sortedClients[i]].client->pers.multikillDisplayed == qfalse){
			if(level.time > (g_multikillTime.integer
				+ g_entities[level.sortedClients[i]].client->pers.multikill_time)){
				G_check_multikill(&g_entities[level.sortedClients[i]],
				G_highest_multikill(&g_entities[level.sortedClients[i]]));
				g_entities[level.sortedClients[i]].client->pers.multikillDisplayed = qtrue;
			}
		}
	}
...
void InitCensorStructure( void )
{
	...
	for ( i = 0; g_censor.string[i] != '\0'; i++ ) {
		if ( g_censor.string[i] == ' ' || g_censor.string[i] == ',' ) {
			g_censor.string[i] = '\0';
			censorDictionary.num_nulled_words++;
		}
	}
	if ( g_censor.string[0] != '\0' )
		censorDictionary.num_nulled_words++;
...
void InitCensorNamesStructure( void )
{
	...
	for ( i = 0; g_censorNames.string[i] != '\0'; i++ ) {
		if ( g_censorNames.string[i] == ' ' || g_censorNames.string[i] == ',' ) {
			g_censorNames.string[i] = '\0';
			censorNamesDictionary.num_nulled_words++;
		}
	}
	if ( g_censorNames.string[0] != '\0' )
		censorNamesDictionary.num_nulled_words++;
...
void G_mapvoteinfo_write()
{
	...
	trap_FS_FOpenFile("mapvoteinfo.cfg", &f, FS_WRITE);
	...
	for( i = 0; i < MAX_VOTE_MAPS; i++) {
		if ( level.mapvoteinfo[i].bspName[0] ) {
			...
			G_shrubbot_writeconfig_string(
					level.mapvoteinfo[i].bspName, f);
			trap_FS_Write("times_played     = ", 19, f);
			G_shrubbot_writeconfig_string(
					va("%d",level.mapvoteinfo[i].timesPlayed), f);
			trap_FS_Write("last_played      = ", 19, f);
			G_shrubbot_writeconfig_string(
					va("%d",level.mapvoteinfo[i].lastPlayed), f);
			trap_FS_Write("total_votes      = ", 19, f);
			G_shrubbot_writeconfig_string(
					va("%d",level.mapvoteinfo[i].totalVotes), f);
			trap_FS_Write("vote_eligible    = ", 19, f);
			G_shrubbot_writeconfig_string(
					va("%d",level.mapvoteinfo[i].voteEligible), f);
...
void G_mapvoteinfo_read()
{
	...
	len = trap_FS_FOpenFile("mapvoteinfo.cfg", &f, FS_READ) ;
	...
	*(cnf + len) = '\0';
	...
	while(*t) {
		...
		else if(!Q_stricmp(t, "times_played")) {
			G_shrubbot_readconfig_int(&cnf,
				&level.mapvoteinfo[curMap].timesPlayed);
		}
		else if(!Q_stricmp(t, "last_played")) {
			G_shrubbot_readconfig_int(&cnf,
				&level.mapvoteinfo[curMap].lastPlayed);
		}
		else if(!Q_stricmp(t, "total_votes")) {
			G_shrubbot_readconfig_int(&cnf,
				&level.mapvoteinfo[curMap].totalVotes);
		}
		else if(!Q_stricmp(t, "vote_eligible")) {
			G_shrubbot_readconfig_int(&cnf,
				&level.mapvoteinfo[curMap].voteEligible);
		}
		else if(!Q_stricmp(t, "[mapvoteinfo]")) {
			// do nothing for the moment
		} else {
			G_Printf("mapvoteinfo: [mapvoteinfo] parse error near "
				"%s on line %d\n",
				t,
