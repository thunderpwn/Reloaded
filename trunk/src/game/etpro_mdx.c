...
void mdx_gentity_to_grefEntity(gentity_t *ent, grefEntity_t *refent, int lerpTime)
{
	...
	if (ent->s.eType == ET_PLAYER) {
		character = BG_GetCharacter(ent->client->sess.sessionTeam, ent->client->sess.playerType, CHARTYPE_NORMAL);
	} else {
		character = BG_GetCharacter(BODY_TEAM(ent), BODY_CLASS(ent), CHARTYPE_NORMAL);
	}
