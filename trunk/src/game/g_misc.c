...
void Fire_Lead_Ext( gentity_t *ent, gentity_t *activator, float spread, int damage, vec3_t muzzle, vec3_t forward, vec3_t right, vec3_t up, int mod ) {
	...
	// antilag code moved to g_active.c under the EV_FIREs
	...
	if( traceEnt->takedamage && traceEnt->client ) {
		...
		if (g_tactics.integer && 
				!activator->client->ps.stats[STAT_AIMING] &&
				traceEnt->health > 0 ) {
...
void mg42_fire( gentity_t *other ) {
	...
	//Perro - tunable
	Fire_Lead( self, other, MG42_SPREAD_MP, g_dmgMG.integer, muzzle, forward, right, up );
...
void mg42_think (gentity_t *self) {
	...
	{
		owner->client->ps.persistant[PERS_HWEAPON_USE] = 0;
		if(!owner->client->pmext.poisoned) {
			owner->client->ps.viewlocked = 0;	// let them look around
		} else {
			owner->client->ps.viewlocked = 2;
			owner->client->ps.viewlocked_entNum = owner->s.number;
		}
		owner->active = qfalse;
...
int QDECL G_SortPlayersByBinocs( const void* a, const void* b )
{
	...
	aBinocs = cla->sess.numBinocs;
	bBinocs = clb->sess.numBinocs;
...
void G_BinocMasters(qboolean endOfMatch)
{
	...
	if ( countConn > 0 ) {
		...
		if(cl->sess.numBinocs > 0) {
			Q_strcat(msg, sizeof(msg), 
					va("^7Binoc Masters: %s^7: %d", 
						cl->pers.netname,
						cl->sess.numBinocs));
		}
	}
...
int QDECL G_SortPlayersByStreak( const void* a, const void* b )
{
	...
	aStreak = cla->sess.kstreak;
	bStreak = clb->sess.kstreak;
...
void G_MapLongStreak(qboolean endOfMatch)
{
	...
	if (level.maxspree_count > 0) {
		Q_strcat(msg, sizeof(msg), 
			va("^8Longest Killing Spree: ^6%d^8 kills by ^7%s^8.", 
