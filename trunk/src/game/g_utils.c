...
void G_ParseCampaigns( void ) {
	...
	if(!mapFound) {
		...
		numdirs = trap_FS_GetFileList("scripts",
			".campaign", bigTextBuffer, sizeof(bigTextBuffer) );
...
qboolean IsVisible( gentity_t *ent1, gentity_t *ent2 ) {
	...
	VectorCopy( ent1->r.currentOrigin, eyeorigin );
	eyeorigin[2] += ent1->client->ps.viewheight;

	// josh: Check from player's view instead of origin
	// use r instead since that was just backwards reconciled
	trap_Trace (&trace, eyeorigin, NULL, NULL, ent2->r.currentOrigin, ent1->s.number, MASK_SHOT );

	if( &g_entities[ trace.entityNum ] == ent2 ) {
		return qtrue;
	}
...
// Neil Toronto (v. josh) - finds the next entity within the radius squared
// starts at *from (example in g_missile.c/G_HomingMissile)
