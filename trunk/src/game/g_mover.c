...
void func_explosive_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	// tjw: don't actually explode, just test scripting
	if(level.testEndRound)
		return;
	// tjw: make the parent (trigger) die too
	// cs: maps like stalingrad are using the same scriptName for both
...
void SP_func_explosive (gentity_t *ent)
{
	...
	// tjw: etpromapscripts break this by using AXIS_OBJECTIVE(1)
	//      as one of the spawnflags for func_explosive.  
	// tjw: implemented dirty dirty hack to support the etpromapscripts
	//      as is, but not drop EXPLOSIVE_START_INVIS compatibility.
	// forty: better to just check if EF_FAKEBMODEL is set in eFlags
	//		  gives a lot more flexibility in map scripts.
	if (
		ent->spawnflags & EXPLOSIVE_START_INVIS &&
...
void G_LinkDebris( void ) {
	...
	for(i = 0; i < level.numDebrisChunks; i++) {
		...
		if(!target) {
			...
			G_Error( "ERROR: func_debris with no target (%s)",
				debris->target );
		}
